...
def fib_sum(n)
  ...
  return 1 if n == 1
  sequence = [1, 1]
  sequence << sequence[-1] + sequence[-2] until sequence.length == n
  sequence.reduce(:+)
end
...
def five_ws(string)
  five_dubs = %w(who what where when why)
  string.split('.').map do |sent|
    if sent.split(' ').any?{|word| five_dubs.include?(word.downcase)}
      sent << '?'
    else
      sent << '.'
    end
...
def cat_pairs(cat_names, cat_price)
  ...
  result = valid_cats.min_by(2){|cat, price| price}.map{|cat| cat[0]}
  cat_names.index(result[0]) > cat_names.index(result[1]) ? result.reverse : result
end
